...
3. Prefer repository-owned facts over prose copies:
   `cli-tools/nddev_cursor_cli.py`, `cli-tools/validate_public_contracts.py`,
   `config/nddev-contract.json`, `build/manifest.json`, and
   `references/cursor-cli-baseline.json`.
4. Keep volatile versions, artifact pins, supported profile lists, and block
   lists code-owned; link to their owner rather than restating them in new docs.
